...
class Ui_Dialog_equipment(object):
    def setupUi(self, Dialog_equipment):
        cursor = connection.cursor()
        with connection.cursor() as cursor:
            count = "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_NAME='equipment';"
            cursor.execute(count)
            colums = cursor.fetchall()
            connection.commit()
        count = len(colums)
        count = count + 2
        Dialog_equipment.setObjectName("Dialog_equipment")
        Dialog_equipment.resize(1141, 751)
        self.tableWidget = QtWidgets.QTableWidget(Dialog_equipment)
        ...
        while i < count:
            ...
            i = i + 1
        self.pushButton_add_equipment = QtWidgets.QPushButton(Dialog_equipment)
        self.pushButton_add_equipment.setGeometry(QtCore.QRect(10, 10, 141, 23))
        self.pushButton_add_equipment.setObjectName("pushButton_add_equipment")
        self.pushButton_back = QtWidgets.QPushButton(Dialog_equipment)
        self.pushButton_back.setGeometry(QtCore.QRect(1050, 10, 75, 23))
        self.pushButton_back.setObjectName("pushButton_back")

        self.retranslateUi(Dialog_equipment)
        QtCore.QMetaObject.connectSlotsByName(Dialog_equipment)

    def retranslateUi(self, Dialog_equipment):
        _translate = QtCore.QCoreApplication.translate
        Dialog_equipment.setWindowTitle(_translate("Dialog_equipment", "Оборудование"))
        self.pushButton_add_equipment.setText(_translate("Dialog_equipment", "Добавить оборудование"))
        self.pushButton_back.setText(_translate("Dialog_equipment", "Назад"))
        ...
        item.setText(_translate("Dialog_equipment", "Кнопка"))
        item = self.tableWidget.horizontalHeaderItem(count)
        item.setText(_translate("Dialog_equipment", "Удалить"))
        cursor = connection.cursor()
        with connection.cursor() as cursor:
            show_query = "SELECT * FROM `equipment` "
            ...
            while i < len(colums):
                ...
                item.setText(_translate("Dialog_equipment", l[i]))
                i = i + 1
            ...
            def Lets():
                ...
                with connection.cursor() as cursor:
                    while i < wr:
                        update_query = "UPDATE `equipment` SET " + l[i] + "= '%s' WHERE idequipment = '%s';" % (self.tableWidget.item(numb_row, i).text(), self.tableWidget.item(numb_row, 0).text())
            ...
            def Del():
                ...
                with connection.cursor() as cursor:
                    while i < wr:
                        delete_query = "DELETE FROM `equipment` WHERE idequipment = %s" % (self.tableWidget.item(numb_row, 0).text())
