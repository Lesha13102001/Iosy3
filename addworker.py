...
class Ui_Dialog_addworker(object):
    def setupUi(self, Dialog_addworker):
        ...
        count = len(colums)
        count = count + 2
        Dialog_addworker.setObjectName("Dialog_addworker")
        Dialog_addworker.resize(1141, 709)
        self.tableWidget = QtWidgets.QTableWidget(Dialog_addworker)
        ...
        self.err = 0
        self.retranslateUi(Dialog_addworker)
        QtCore.QMetaObject.connectSlotsByName(Dialog_addworker)

    def retranslateUi(self, Dialog_addworker):
        _translate = QtCore.QCoreApplication.translate
        Dialog_addworker.setWindowTitle(_translate("Dialog_addworker", "Добавить рабочего"))
        cursor = connection.cursor()
        with connection.cursor() as cursor:
        ...
        item.setText(_translate("Dialog_addworker", "Добавить"))
        item = self.tableWidget.horizontalHeaderItem(count)
        item.setText(_translate("Dialog_addworker", "Вернуться назад"))
        ...
        def Lets():
            try:
                ...
                cursor = connection.cursor()
                with connection.cursor() as cursor:
                    insert_query = "INSERT INTO `worker` (" + inf_x + ") VALUES (" + inf + ");"
                    print(insert_query)
