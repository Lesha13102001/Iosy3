...
class Ui_Dialog_adddiet(object):
    def setupUi(self, Dialog_adddiet):
        ...
        count = len(colums)
        count = count + 2
        Dialog_adddiet.setObjectName("Dialog_adddiet")
        Dialog_adddiet.resize(1141, 709)
        self.tableWidget = QtWidgets.QTableWidget(Dialog_adddiet)
        ...
        self.err=0
        self.retranslateUi(Dialog_adddiet)
        QtCore.QMetaObject.connectSlotsByName(Dialog_adddiet)

    def retranslateUi(self, Dialog_adddiet):
        _translate = QtCore.QCoreApplication.translate
        Dialog_adddiet.setWindowTitle(_translate("Dialog_adddiet", "Добавить"))
        ...
        item.setText(_translate("Dialog_adddiet", "Добавить"))
        item = self.tableWidget.horizontalHeaderItem(count)
        item.setText(_translate("Dialog_adddiet", "Вернуться назад"))

        def Lets():
            try:
                if int(self.tableWidget.item(0, 3).text()) < 2:
                    self.err = 1
                    add = QMessageBox()
                    add.setWindowTitle("Ошибка")
                    add.setText("Кол-во приемов пищи в день не может быть меньше 2-ух")
                ...
                cursor = connection.cursor()
                with connection.cursor() as cursor:
                    insert_query = "INSERT INTO `diet` (" + inf_x + ") VALUES (" + inf + ");"
                    print(insert_query)
